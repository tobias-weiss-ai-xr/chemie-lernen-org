...
function calculatePH(analyteType, analyteConc, analyteVol, titrantType, titrantConc, titrantVol, pka = 0, pkb = 0) {
...
function runTitration() {
  ...
  const analytepKa = parseFloat(document.getElementById('analyte-pka').value);
  ...
  const titrationStep = parseFloat(document.getElementById('titration-step').value);
  const titrantpKb = parseFloat(document.getElementById('titrant-pkb').value);

  if (!analyteConc || !analyteVol || !titrantConc || !titrationStep) {
    showError('Bitte füllen Sie alle Pflichtfelder aus.');
    return;
  }

  if (analyteConc <= 0 || analyteVol <= 0 || titrantConc <= 0 || titrationStep <= 0) {
    showError('Alle Werte müssen positiv sein.');
    return;
  }

  if (analyteType === 'weak-acid' && !analytepKa) {
    showError('Bitte geben Sie den pKa-Wert für die schwache Säure ein.');
    return;
  }

  if (titrantType === 'weak-base' && !titrantpKb) {
    showError('Bitte geben Sie den pKb-Wert für die schwache Base ein.');
    return;
  }
  ...
  try {
    ...
    for (let v = 0; v <= maxVolume; v += titrationStep) {
      const ph = calculatePH(analyteType, analyteConc, analyteVol, titrantType, titrantConc, v, analytepKa, titrantpKb);
      dataPoints.push({ volume: v, ph: ph });
    }

    // Ensure equivalence point is included
    const equivPH = calculatePH(analyteType, analyteConc, analyteVol, titrantType, titrantConc, equivVolume, analytepKa, titrantpKb);
    if (!dataPoints.some(p => Math.abs(p.volume - equivVolume) < titrationStep / 2)) {
    ...
    // Display results
    displayResults(analyteType, titrantType, analyteConc, analyteVol, titrantConc, equivVolume, equivPH, dataPoints, analytepKa, titrantpKb);
...
// Display results
function displayResults(analyteType, titrantType, analyteConc, analyteVol, titrantConc, equivVolume, equivPH, dataPoints, pka, pkb) {
  // Summary
  const typeNames = {
  ...
  titrationChart = new Chart(chartCtx, {
    ...
    data: {
      ...
      datasets: [{
        ...
        pointBackgroundColor: dataPoints.map(p => Math.abs(p.volume - equivVolume) < 0.5 ? 'red' : 'rgb(75, 192, 192)'),
        tension: 0.4,
        fill: true
      }, {
