...
- [x] 1.1 Create `api/services/tool-router.js`: define `TOOL_REGISTRY` array
...
- [x] 2.1 Implement `resolveTool(bloomsIndex, objectiveTags?)` in
...
- [x] 2.2 Implement `getAllTools(bloomsIndex?, tags?)` for the editorial
      endpoint: returns all matching entries (or full registry if no filters).

## 3. Strategy activator enhancement

- [x] 3.1 Import `resolveTool` into `api/services/zpd-engine.js`. Update
...
- [x] 3.2 Ensure `recommendedStrategy()` accepts optional `objectiveTags`
      parameter (passed from route).

## 4. API routes

- [x] 4.1 Update `GET /api/learning-paths/:slug/next` in
...
- [x] 4.2 Create `api/routes/tools.js`: `GET /api/tools` with optional query
      params `?bloom=<level>&tags=<csv>` calling `getAllTools()`. Auth
      optional (editorial use). Returns `{ tools: [...] }`.
- [x] 4.3 Register `api/routes/tools.js` in the Express app (server.js or
      route index).

## 5. Tests

- [x] 5.1 `tests/tool-router.test.mjs`: pure-function unit tests for
...
- [x] 5.2 `tests/zpd-engine-tool-strategy.test.mjs`: test that
