...
- [x] 1.1 Create `api/services/scaffolding-engine.js` (public): export
      `BLOOM_METADATA` (array of `{level, verb, descriptor, hintType}` per
      Bloom index 1–6).
- [x] 1.2 Implement `computeStaircase(bloomsMaxReached, targetBloom)` pure
      function: returns array of steps from `bloomsMaxReached + 1` to
      `targetBloom`. Empty if `bloomsMaxReached >= targetBloom`. Truncate
      to max 5 steps.
- [x] 1.3 Implement `scaffoldingPlan(userId, objectiveSlug)` async: reads
...
- [x] 2.1 Create `api/routes/scaffolding.js`: `GET /api/scaffolding/hints`
      (query param `objectiveSlug`). Auth required. Returns scaffolding plan
      or 404. Register route in `api/server.js`.

## 3. Tests

- [x] 3.1 `tests/scaffolding-engine.test.mjs`: DB-free unit tests:
