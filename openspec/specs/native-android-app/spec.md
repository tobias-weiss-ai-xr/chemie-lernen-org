...
Native Android client (Kotlin + Jetpack Compose) for chemie-lernen.org in `android/`. Consumes the existing REST API with zero server changes; mirrors web behavior for quizzes, grading, offline sync, FSRS review and gamification. See `android/README.md`.
