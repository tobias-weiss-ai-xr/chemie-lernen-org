...
## Premium Content (Lesson Plans & Worksheets)

All premium-content endpoints require a valid JWT from a premium-tier user
(`requirePremium` middleware). Returns `402` for free users, `401` for
unauthenticated requests. Content is generated by the LiteLLM model configured
via `LITELLM_MODEL_PREMIUM`, enriched with knowledge-graph context where
available.

### `POST /api/premium/lesson-plan`

Generate a structured lesson plan (Unterrichtsplan) for a single class period.
...
| -------------- | -------- | -------------------------------------- | -------- |
| `topic`        | yes      | string, min. 2 chars                   | —        |
| `klassenstufe` | no       | `8`, `9`, `10`, `11`, `12`, `13`       | `10`     |
| `duration`     | no       | `15`, `30`, `45`, `60`, `90`           | `45`     |
| `difficulty`   | no       | `einfach`, `mittel`, `fortgeschritten` | `mittel` |
...
```json
{
  "lessonPlan": {
    ...
    "priorKnowledge": ["Voraussetzung 1"],
    ...
    "differentiation": { "stronger": "...", "weaker": "..." },
...
**Errors:** `400` (invalid input), `502` (LiteLLM unavailable or unparseable
response — includes a `raw` field), `500` (server error).

### `POST /api/premium/worksheet`

Generate a worksheet (Arbeitsblatt) with mixed exercise types.
...
```json
{
  "topic": "Säuren und Basen",
  "exerciseCount": 5,
  "types": ["multiple-choice", "berechnung", "lueckentext"]
}
```
...
| --------------- | -------- | ---------------------------------------------------------------------- | ----------------------------------- |
| `topic`         | yes      | string, min. 2 chars                                                   | —                                   |
| `exerciseCount` | no       | number 1–20                                                            | `5`                                 |
| `types`         | no       | array of `multiple-choice`, `lueckentext`, `berechnung`, `kurzantwort` | `["multiple-choice", "berechnung"]` |
...
**Errors:** `400` (invalid `topic` or empty `types`), `502` (LiteLLM
unavailable / unparseable array), `500` (server error).
