...
describe('Mass-Mass Stoichiometry (calc-massmass.js)', () => {
  ...
  test('water formation: 4g H₂ + O₂ → H₂O', () => {
    const { n1, n2, m2 } = calcMassMass(4, 2, 18, 2, 2);
    expect(n1).toBe(2);
    expect(n2).toBe(2);
    expect(m2).toBe(36);
  });
  ...
  test('methane combustion: 16g CH₄ → CO₂', () => {
    const { n1, n2, m2 } = calcMassMass(16, 16, 44, 1, 1);
    expect(n1).toBe(1);
    expect(n2).toBe(1);
    expect(m2).toBe(44);
  });
  ...
  test('ammonia (Haber): 28g N₂ → NH₃', () => {
    const { n1, n2, m2 } = calcMassMass(28, 28, 17, 1, 2);
    expect(n1).toBe(1);
    expect(n2).toBe(2);
    expect(m2).toBe(34);
  });
  ...
  test('photosynthesis: 264g CO₂ → glucose', () => {
    const { n1, n2, m2 } = calcMassMass(264, 44, 180, 6, 1);
    expect(n1).toBe(6);
  ...
  test('conservation: mass in = mass out only if M₁ = M₂ and v₁ = v₂', () => {
    const { m2 } = calcMassMass(10, 50, 50, 1, 1);
    expect(m2).toBe(10);
  });

  test('different molar masses change mass ratio', () => {
    const { m2 } = calcMassMass(10, 2, 18, 1, 1);
    expect(m2).toBe(90);
...
describe('Percent Yield (calc-yield.js)', () => {
  function calcYieldPct(theo, act) {
    return (act / theo) * 100;
  }

  test('100% yield (theoretical = actual)', () => {
    expect(calcYieldPct(50, 50)).toBe(100);
  });

  test('50% yield', () => {
    expect(calcYieldPct(40, 20)).toBe(50);
  });

  test('75% yield', () => {
    expect(calcYieldPct(80, 60)).toBe(75);
  });

  test('0% yield (no product)', () => {
    expect(calcYieldPct(100, 0)).toBe(0);
  });

  test('over 100% yield (impure product)', () => {
    expect(calcYieldPct(50, 60)).toBeGreaterThan(100);
    expect(calcYieldPct(50, 60)).toBe(120);
  });

  test('typical organic synthesis yield (30-80%)', () => {
    const result = calcYieldPct(10, 4.5);
...
describe('Limiting Reagent (calc-limiting.js)', () => {
  function findLimiting(m1, M1, m2, M2) {
    const n1 = m1 / M1;
    const n2 = m2 / M2;
    return {
      n1,
      n2,
      limiting: n1 < n2 ? 1 : 2,
      excess: n1 < n2 ? n2 - n1 : n1 - n2,
    };
  }

  test('reagent 1 is limiting', () => {
    const r = findLimiting(5, 50, 10, 50);
  ...
  test('reagent 2 is limiting', () => {
    const r = findLimiting(10, 50, 5, 50);
  ...
  test('equal moles — reagent 2 wins (n1 < n2 is false when equal)', () => {
    const r = findLimiting(10, 50, 10, 50);
  ...
  test('excess calculation is correct', () => {
    const r = findLimiting(2, 40, 6, 60);
  ...
  test('different molar masses — smaller mass can be in excess', () => {
    const r = findLimiting(2, 2, 50, 50);
    expect(r.n1).toBe(1);
    expect(r.n2).toBe(1);
    expect(r.excess).toBe(0);
  });
  ...
  test('one reagent has vastly more moles', () => {
    const r = findLimiting(1, 100, 100, 1);
