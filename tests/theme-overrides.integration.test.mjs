 ...
 *
 * Integration test: end-to-end GET/PUT against a live Express app
 * using the theme-overrides router with auth.js + neo4j mocked.
 *
 * Verifies:
 *   - GET /api/theme-overrides returns the stored map ({} if none)
 *   - PUT /api/theme-overrides replaces the map, returns it
 *   - PUT validates body: keys and values must be non-empty strings
 *   - Admin key is enforced (401 missing/wrong, 503 not configured)
 */
...
describe('GET /api/theme-overrides', () => {
  test('returns empty object when no overrides are stored', async () => {
  ...
  test('returns stored overrides after a PUT', async () => {
    ...
    // Seed via PUT
    const putRes = await fetch(`${baseURL}/api/theme-overrides`, {
      method: 'PUT',
...
describe('PUT /api/theme-overrides', () => {
  ...
  test('fully replaces the previous map (not merge)', async () => {
    ...
    // Second PUT with completely different keys
    const res = await fetch(`${baseURL}/api/theme-overrides`, {
      method: 'PUT',
      headers: adminHeaders(),
