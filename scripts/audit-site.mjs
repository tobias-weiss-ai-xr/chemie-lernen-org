...
const pages = [
  ...
  { url: '/themenbereiche/stoichiometrie/', name: 'themenbereiche-stoichiometrie', label: 'Themenbereich: Stoichiometrie' },
  { url: '/themenbereiche/chemische-bindungen/', name: 'themenbereiche-bindungen', label: 'Themenbereich: Bindungen' },
...
for (const page of pages) {
  ...
  try {
    ...
    const placeholderPatterns = [
      /lorem ipsum/i, /placeholder/i, /coming soon/i, /in bearbeitung/i,
      /under construction/i, /dummy/i, /beispiel.*text/i, /TODO/i, /FIXME/i,
      /wird bald.*hinzugefügt/i, /no content available/i, /inhalt folgt/i,
      /dieser bereich wird/i, /noch nicht verfügbar/i,
    ];
    ...
    if (foundPlaceholders.length > 0) {
      ...
      foundPlaceholders.forEach((r) => console.log(`    Pattern "${r.pattern}": ${r.matches.length} matches`));
    }

    // Main content size
    const mainContent = await p.$eval('main, .content, article, #content', (el) => ({
      textLength: el.innerText.trim().length,
      childCount: el.children.length,
    })).catch(() => ({ textLength: 0, childCount: 0, error: 'no main element' }));
    console.log(`  Main content: ${mainContent.textLength} chars, ${mainContent.childCount} children ${mainContent.error ? `(${mainContent.error})` : ''}`);

    // Empty visible sections
    const emptySections = await p.$$eval('section, .card, .container, div[class*="section"], div[class*="card"]', (els) =>
      els
        .filter((el) => {
    ...
    if (emptySections.length > 0) {
      ...
      emptySections.forEach((s) => console.log(`    <${s.tag} class="${s.class}"> h=${s.height}px text="${s.text}"`));
    }

    // Stuck loaders
    const spinners = await p.$$eval('[class*="loading"], [class*="spinner"], [class*="skeleton"]', (els) =>
      els
    ...
    if (page.name === 'wissennetz') {
      ...
      const searchInput = await p.$('input[type="search"], input[placeholder*="Such"], input[placeholder*="search"], input');
    ...
    if (page.name === 'ki-assistent') {
      ...
      const chatMessages = await p.$$eval('[class*="chat"], [class*="message"], [class*="response"]', (els) => ({
        count: els.length,
    ...
    if (page.name === 'fortschritt') {
      const progressBars = await p.$$eval('[class*="progress"], [role="progressbar"]', (els) => els.length);
    ...
    if (page.name === 'strassenszene') {
      const roadmapItems = await p.$$eval('[class*="roadmap"], [class*="stage"], [class*="milestone"], [class*="phase"]', (els) => els.length);
      const listItems = await p.$$eval('li', (els) => els.length);
      const svgs = await p.$$eval('svg', (els) => els.length);
      console.log(`  [Strassenszene] Roadmap elements: ${roadmapItems}, list items: ${listItems}, SVGs: ${svgs}`);
    }
    ...
    if (consoleLogs.length > 0) {
      ...
      consoleLogs.slice(0, 10).forEach((log) => console.log(`    [${log.type}] ${log.text.substring(0, 150)}`));
    }

    // Network errors
    if (networkErrors.length > 0) {
      console.log(`  NETWORK ERRORS (${networkErrors.length}):`);
      networkErrors.forEach((err) => console.log(`    ${err.url.substring(0, 80)} -> ${err.error}`));
    }
