#!/bin/bash
# Backup script for leads.graphwiz.ai Neo4j knowledge graph
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="$(dirname "$SCRIPT_DIR")"
CONTAINER_NAME="leads-neo4j"
DATABASE_NAME="neo4j"
NEO4J_PASSWORD="${NEO4J_PASSWORD:-password}"

BACKUP_DIR="${PROJECT_DIR}/backups/neo4j-leads"
LOG_DIR="${PROJECT_DIR}/backups/logs"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
BACKUP_FILE="leads_knowledge_backup_${TIMESTAMP}.dump"
LOG_FILE="${LOG_DIR}/backup_leads_${TIMESTAMP}.log"
...
check_prerequisites() {
    ...
    if ! docker ps --format '{{.Names}}' | grep -q "^${CONTAINER_NAME}$"; then
        log "WARNING" "Container ${CONTAINER_NAME} is not running — skipping leads backup"
        exit 0
    fi
...
create_backup() {
    ...
    if docker exec "${CONTAINER_NAME}" neo4j-admin database dump "${DATABASE_NAME}" \
        ...
        ln -sf "$BACKUP_FILE" "${BACKUP_DIR}/leads_knowledge_latest.dump"
        local success=true
    else
...
cleanup_old_backups() {
    ...
    find "$BACKUP_DIR" -name "leads_knowledge_backup_*.dump" -mtime +${KEEP_DAILY} -delete 2>/dev/null || true
