...
function addReactionStep() {
  ...
  stepDiv.innerHTML =
    ...
    '<i class="fa fa-step-forward"></i> Schritt ' +
    stepCounter +
    '</h4>' +
    '<button class="btn btn-sm btn-danger" onclick="removeStep(' +
    stepCounter +
    ')">' +
    ...
    '<input type="number" class="form-control step-coeff-r" id="step-' +
    stepCounter +
    '-coeff-r" value="1" step="any" min="0">' +
    '</div>' +
    '</div>' +
    '<div class="col-md-3">' +
    '<div class="form-group">' +
    '<label>Koeffizient Produkt:</label>' +
    '<input type="number" class="form-control step-coeff-p" id="step-' +
    stepCounter +
    '-coeff-p" value="1" step="any" min="0">' +
    '</div>' +
    '</div>' +
    '<div class="col-md-3">' +
    '<div class="form-group">' +
    '<label>Molare Masse Produkt (g/mol):</label>' +
    '<input type="number" class="form-control step-molar-mass" id="step-' +
    stepCounter +
    '-molar-mass" step="any" placeholder="Optional">' +
    '</div>' +
    '</div>' +
    '<div class="col-md-3">' +
    '<div class="form-group">' +
    '<label>Produkt-Verbindung:</label>' +
    '<input type="text" class="form-control step-product" id="step-' +
    stepCounter +
    '-product" placeholder="z.B. FeO">' +
    '</div>' +
    '</div>' +
    '</div>' +
    '<div class="form-group">' +
    '<label>Reaktionsgleichung (optional, zur Anzeige):</label>' +
    '<input type="text" class="form-control step-equation" id="step-' +
    stepCounter +
    '-equation" placeholder="z.B. 2Fe + O2 \u2192 2FeO">' +
    '</div>' +
    '</div>';
...
function calculateMultiStepPure(initialAmount, steps) {
  ...
  steps.forEach((step, index) => {
    const coeffR = step.coeffR;
    const coeffP = step.coeffP;
    const molarMass = step.molarMass;
    const product = step.product || 'Produkt ' + (index + 1);
...
function calculateMultiStep() {
  ...
  if (isNaN(initialAmount) || initialAmount <= 0) {
    showToast(
      'Bitte geben Sie eine g\u00fcltige Stoffmenge f\u00fcr den Ausgangsstoff ein.',
      'error'
    );
    return;
  }
...
function displayMultiStepResults(initialAmount, initialMolarMass, initialCompound, results) {
  ...
  html +=
    '<h4 style="color: #2E7D32; margin-top: 0;"><i class="fa fa-play-circle"></i> Ausgangsstoff</h4>';
  ...
  results.forEach((result, index) => {
    ...
    html +=
      '<p><strong>Koeffizienten:</strong> \u03bd\u2081 = ' +
      result.coeffR +
      ', \u03bd\u2082 = ' +
      result.coeffP +
      '</p>';
    html +=
      '<p><strong>Berechnung:</strong> ' +
      _escapeHtml(result.product) +
      ' = ' +
      result.reactantAmount.toFixed(4) +
      ' \u00d7 (' +
      result.coeffP +
      '/' +
      result.coeffR +
      ')</p>';
    html +=
      '<p><strong>Produktstoffmenge:</strong> <strong style="color: ' +
      (index % 2 === 0 ? '#E65100' : '#C2185B') +
      '">' +
      result.productAmount.toFixed(4) +
      ' mol</strong></p>';

    if (result.productMass !== null) {
      html +=
        '<p><strong>Produktmasse:</strong> <strong style="color: ' +
        (index % 2 === 0 ? '#E65100' : '#C2185B') +
        '">' +
        result.productMass.toFixed(4) +
        ' g</strong> (' +
        result.molarMass +
        ' g/mol)</p>';
    }
  ...
  html +=
    ...
    ' mol)</p>' +
    '<p style="font-size: 18px; margin-top: 15px;"><strong>Gesamtausbeute:</strong> ' +
  ...
  if (finalResult.productMass !== null && !isNaN(initialMolarMass) && initialMolarMass > 0) {
    ...
    html +=
      '<p style="font-size: 16px;"><strong>Massenausbeute:</strong> ' +
      massYield.toFixed(2) +
      '%</p>';
  }
  ...
  const historyData =
    ...
    lastResult.product +
    ': ' +
    lastResult.productAmount.toFixed(4) +
    ' mol (' +
    results.length +
    ' Schritte)';
...
function exportMultiStepToPDF() {
  ...
  stepElements.forEach((stepEl, index) => {
    ...
    if (molarMass) {
      ...
      doc.text('Masse: ' + prodMass + ' g (' + molarMass + ' g/mol)', 25, y);
      y += 8;
    }
