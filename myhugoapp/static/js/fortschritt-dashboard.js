...
async function loadModuleProgress() {
  ...
  if (Object.keys(stats).length === 0) {
    container.innerHTML =
      '<p class="text-muted">Noch keine Daten vorhanden. Beginne mit \u00dcbungen, um deinen Fortschritt zu sehen.</p>';
    return;
  }
...
async function loadAchievements() {
  ...
  const allAchievements = [
    ...
    },
    {
      ...
      max: 100,
      current: () => Math.min(10, allData.length),
      of: 10,
    },
    {
      id: 'fifty_exercises',
      ...
      max: 100,
      current: () => Math.min(50, allData.length),
      of: 50,
    },
    {
      id: 'perfect_score',
      ...
      max: 100,
      current: () => (allData.some((e) => e.total > 0 && e.correct === e.total) ? 1 : 0),
      of: 1,
    },
    {
      id: 'streak_3',
    ...
    },
    {
      ...
      max: 100,
      current: () => Math.min(20, allData.length),
      of: 20,
    },
    {
      id: 'night_owl',
      ...
      progress: () => {
        return allData.some((e) => {
          const h = new Date(e.lastAttempt).getHours();
          return h >= 22 || h < 5;
        })
          ? 100
          : 30;
    ...
    },
    {
      ...
      max: 100,
      current: () => Math.min(5, earned.length),
      of: 5,
    },
    {
      id: 'speed_demon',
      ...
      progress: () => {
        ...
        return firstToday.some((e) => {
          const t = new Date(e.lastAttempt);
          return t.getHours() === 0 && t.getMinutes() <= 5;
        })
          ? 100
          : 60;
      },
    ...
    },
    {
      ...
      max: 100,
      current: () => Math.min(11, earned.length),
      of: 11,
    },
  ];
...
async function loadPerAreaProgress() {
  ...
  const overallPct =
    areaProgressData.reduce((sum, area) => sum + area.pct, 0) / tracking.areas.length;
  ...
  container.innerHTML =
    ...
    sorted
      .map((entry) => {
        ...
        const result = entry.completed
          ? '<span class="label label-success">Erledigt</span>'
