 ...
 * Optimizes bundle loading with dynamic imports and code splitting
 */
...
class LazyLoader {
  ...
  async loadCalculator(calculatorType) {
    if (this.loadedModules.has(calculatorType)) {
      return this.cache.get(calculatorType);
    }

    if (this.loadingPromises.has(calculatorType)) {
      return this.loadingPromises.get(calculatorType);
    }
    ...
    const loadPromise = this.dynamicImportWithFallback(modulePath);
    this.loadingPromises.set(calculatorType, loadPromise);
    ...
    try {
      ...
      this.loadingPromises.delete(calculatorType);
      console.error(`Failed to load calculator ${calculatorType}:`, error);
      throw error;
    }
  }
  ...
  scriptInjectionFallback(modulePath) {
