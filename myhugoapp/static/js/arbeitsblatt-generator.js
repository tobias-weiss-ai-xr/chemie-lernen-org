...
(function () {
  ...
  function genStoichiometry(diff) {
    var elements = [
      ...
      { name: 'Sauerstoff', symbol: 'O2', mass: 32.00 },
    ...
    if (diff === 1) {
      ...
      return {
        question: 'Berechnen Sie die Masse von ' + moles + ' mol ' + sub.name + ' (' + sub.symbol + ').',
        answer: mass + ' g',
      ...
      return {
        question: 'Wie viel mol ' + sub.name + ' (' + sub.symbol + ') sind in ' + mass + ' g enthalten?',
        answer: moles + ' mol',
        steps: 'n = m / M = ' + mass + ' g / ' + sub.mass + ' g/mol = ' + moles + ' mol'
      };
    } else {
      var sub2 = pick(elements.filter(function (e) { return e.symbol !== sub.symbol; }));
      ...
      return {
        question: massGiven + ' g ' + sub.name + ' (' + sub.symbol + ') reagieren im Verhältnis 1:' + ratio + ' mit ' + sub2.name + ' (' + sub2.symbol + '). Welche Masse an ' + sub2.name + ' wird benötigt?',
        answer: mass2 + ' g',
        steps: 'n(' + sub.symbol + ') = ' + massGiven + ' g / ' + sub.mass + ' g/mol = ' + molesGiven + ' mol\nn(' + sub2.symbol + ') = ' + molesGiven + ' × ' + ratio + ' = ' + roundTo(molesGiven * ratio, 3) + ' mol\nm(' + sub2.symbol + ') = ' + roundTo(molesGiven * ratio, 3) + ' × ' + sub2.mass + ' g/mol = ' + mass2 + ' g'
      };
    }
  }
  ...
  function genMolarMass(diff) {
    var compounds = [
      ...
      { formula: 'NaOH', name: 'Natronlauge', mass: 40.00 },
  ...
  function genPh(diff) {
    ...
    if (diff <= 2) {
      ...
      return {
        question: 'Berechnen Sie den pH-Wert einer ' + conc + ' mM ' + acid.name + ' (pKa = ' + acid.pka + ').',
        answer: phValue + '',
        steps: acid.strong
          ? 'pH = -log[H+] = -log(' + (conc / 1000) + ') = ' + phValue
          : 'pH = 0.5 × pKa - 0.5 × log(c) = 0.5 × ' + acid.pka + ' - 0.5 × log(' + (conc / 1000) + ') = ' + phValue
      };
    } else {
      conc = rand(1, 100);
      if (acid.strong) {
      ...
      return {
        question: 'Berechnen Sie den pH-Wert einer ' + conc + ' mM Lösung der Base ' + acid.name + ' (pKa = ' + acid.pka + ').',
        answer: phValue + '',
  ...
  function genRedox(diff) {
    var redoxPairs = [
      ...
      { ox: 'Ag', red: 'Ag+', e0: 0.80, name: 'Silber/Silber(I)' }
    ];

    var pair1 = pick(redoxPairs);
    var pair2 = pick(redoxPairs.filter(function (p) { return p.ox !== pair1.ox; }));
    var cellVoltage = roundTo(Math.abs(pair1.e0 - pair2.e0), 2);

    return {
      question: 'Berechnen Sie die Zellspannung eines galvanischen Elements aus ' + pair1.name + ' (E°=' + pair1.e0 + ' V) und ' + pair2.name + ' (E°=' + pair2.e0 + ' V).',
      answer: cellVoltage + ' V',
      steps: 'ΔE° = |E°(Kathode) - E°(Anode)| = |' + Math.max(pair1.e0, pair2.e0) + ' - ' + Math.min(pair1.e0, pair2.e0) + '| = ' + cellVoltage + ' V'
    };
  }
  ...
  function generateProblem(theme, difficulty) {
    switch (theme) {
      case 'stoichiometry': return genStoichiometry(difficulty);
      case 'balancing': return genBalancing(difficulty);
      case 'molar-mass': return genMolarMass(difficulty);
      case 'ph': return genPh(difficulty);
      case 'redox': return genRedox(difficulty);
      default: return genMixed(difficulty);
    }
  }
  ...
  function renderWorksheet() {
    ...
    metaEl.textContent = 'Schwierigkeit: ' + diffNames[state.difficulty] + ' | ' + state.count + ' Aufgaben';
  ...
  function renderSolutions() {
    ...
    state.solutions.forEach(function (sol) {
      ...
      if (state.solution === 'separate') {
        html += '  <p class="ws-steps"><strong>Rechenweg:</strong><br>' + sol.steps.replace(/\n/g, '<br>') + '</p>';
      }
      html += '</div>';
    });
